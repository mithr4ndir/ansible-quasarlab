...
| Agent command audit | `/usr/local/bin/agent-cmd-audit` plus `agent-cmd-audit.timer` (every 30s, as `ansible_user`) and `agent-cmd-audit-canary.timer` (hourly). Every Bash command a Claude Code session runs on this host is spooled by a PostToolUse / PostToolUseFailure hook (registered in claude-config, not here) and posted to the Discord #cmds channel in batches, grouped by session. Command text is untrusted, so it is redacted and then re-checked: anything still matching a secret pattern is withheld rather than posted, and the arguments of `op`, `gpg`, `vault` and friends are dropped wholesale. The webhook URL comes from the 1Password secret cache at run time, under the slug `discord_cmds_webhook_url.<16 hex chars of sha256 over the reference>`; nothing is posted without it. This is telemetry, not a tamper-proof audit log: the spool and the agent's own settings belong to the user being audited. It also cannot see commands that were blocked before running, the outcome of backgrounded commands, or shell run outside the Bash tool. Only when `cmd_center_cmd_audit_enabled` is true. |
...
- `cmd_center_cmd_audit_enabled` set to `true` in host_vars to install the agent command audit script, its settings file and its two timers. `cmd_center_cmd_audit_op_reference` must name a 1Password item whose cache slug has been seeded first, or the flusher resolves no webhook and posts nothing. `cmd_center_cmd_audit_recording_enabled` stops new commands being recorded while the flusher keeps draining what is already spooled; `touch ~/.local/state/agent-cmd-audit/OFF` does the same immediately, with no deploy. Setting `cmd_center_cmd_audit_enabled` back to `false` stops managing it but does not remove installed timers; disable those by hand with `systemctl disable --now agent-cmd-audit.timer agent-cmd-audit-canary.timer`.
