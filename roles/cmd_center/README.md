...
| Lab changelog | `/usr/local/bin/lab-changelog` plus the `lab-changelog.timer` system timer (daily 23:55 UTC, as `ansible_user`). Posts merged PRs and opened or closed issues from `cmd_center_changelog_repos` to Discord, with optional claude-generated highlights from those titles and bodies only. The daily window starts one hour before the end of the last successful daily run (24 hours back on the first run, capped at 7 days), so missed or failed runs lose nothing. The webhook URL comes from the 1Password secret cache at run time, under the slug `discord_changelog_webhook_url.<16 hex chars of sha256 over the reference>`, so a changed `cmd_center_changelog_op_reference` never reuses the old channel's cached URL; nothing is posted without it. Seeding and refreshing the slug is described in `defaults/main.yml`. Also the target of the Claude Code SessionEnd hook (`lab-changelog session-end`, registered in claude-config). Only when `cmd_center_changelog_enabled` is true, which only command-center1 sets. |
| Lab scout | `/usr/local/bin/lab-scout` plus the `lab-scout.timer` system timer (weekly, Saturday 14:00 UTC, as `ansible_user`). Posts a Radagast-themed digest of at most `cmd_center_scout_max_ideas` new homelab project ideas to Discord. The inventory of what the lab already runs is read from `origin/main` of the local k8s-argocd and ansible-quasarlab checkouts (`git ls-tree`, never the working tree) plus `cmd_center_scout_extra_inventory`, and the lab profile is rendered to `/etc/lab-scout/profile.yaml` from `cmd_center_scout_profile`. claude (`cmd_center_scout_claude_model`, web search and fetch tools only, structured JSON output) proposes candidates; the script drops any idea that fails validation (length, enums, https URL to a public host, no mentions, IP addresses, secret-like text, links or numbers in text), duplicates the inventory or an earlier digest (`/var/lib/lab-scout/seen.json`), or fails its fact check: GitHub repos must exist, not be archived and have been pushed within `cmd_center_scout_max_stale_days`; other links must answer one GET. Stars, last push and license come from `gh api`, never from the model. The webhook URL comes from the 1Password secret cache under the slug `discord_lab_scout_webhook_url.<16 hex chars of sha256 over the reference>`; nothing is posted and claude is not called without it. Writes `lab_scout.prom` (last run, last success, success, ideas posted and dropped) to the node_exporter textfile directory. `lab-scout weekly --dry-run` prints the payload without touching 1Password, Discord, the seen state or metrics. Only when `cmd_center_scout_enabled` is true, which only command-center1 sets. |
...
- `cmd_center_scout_enabled` set to `true` in host_vars to install the lab scout script, its settings, profile, state directory and weekly timer. `cmd_center_scout_op_reference`, `cmd_center_scout_on_calendar`, `cmd_center_scout_claude_model`, `cmd_center_scout_claude_timeout`, `cmd_center_scout_max_ideas`, `cmd_center_scout_max_stale_days`, `cmd_center_scout_inventory`, `cmd_center_scout_extra_inventory` and `cmd_center_scout_profile` tune it. Setting it back to `false` stops managing it but does not remove an installed timer; disable that by hand with `systemctl disable --now lab-scout.timer`.
