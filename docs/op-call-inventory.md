...
| `scripts/run-ups-shutdown.sh` (`cached_op_read nut_monitor_password ...`) | CACHED | `op-secret-cache.sh` | Operator-run wrapper for `ups-shutdown.yml`. Replaced the raw `op read` the playbook used to tell the operator to export. Served stale when op fails or the kill switch is active; with nothing cached it exits before contacting any host. The power-loss shutdown itself never calls op (upsmon reads `/etc/nut/upsmon.conf`). |
| `scripts/run-uptime-kuma.sh` (`cached_op_read discord_alerts_webhook_url ...`) | CACHED | `op-secret-cache.sh` | Operator-run wrapper for `uptime-kuma.yml`. Reads the Discord #alerts webhook (`op://Infrastructure/vausmfy2q2m57r6scvziyrc7lq/credential`, the item discord-alert-proxy's ExternalSecret uses). Served stale when op fails or the kill switch is active; with nothing cached it exits before contacting the host. Uses `inventory.static.ini` only, so no Proxmox token and no dynamic inventory. |
